        ...
        <div className="ml-4 h-screen flex flex-col">
            ...
            <div className="flex-1 max-h-96 flex flex-col gap-2 mt-2 overflow-y-auto overflow-x-hidden">
                {state?.data?.map((item, index) => { 
                    ...
                    const img = item.first_user.id === user?.data.id ? item.second_user.image : item.first_user.image 
                    return(
                        <div onClick={() => handleSelect(item)} key={index} className='hover:bg-gray-100 select-none cursor-pointer rounded-md p-2 flex gap-2 w-full'>
            ...
            {/* Chat Component */}
            <div className="bg-prc rounded-md p-2 flex gap-2 mt-2">
                    <div className="p-4 rounded-full bg-src"
                            style={{
