...
export default function ApplicantSearchComponent() {
    const {apiClient} = useContext(AppContext)
    const [data, setData] =useState({type: 'jobs'})
    ...
    const handleNav = (id) => {
        if(data.type === 'employer'){nav(`/Admin/Accounts/${id}`)}
        if(data.type === 'jobs'){nav(`/applicant/Jobs/search/${id}`)}
    }
    return (
    <div>
